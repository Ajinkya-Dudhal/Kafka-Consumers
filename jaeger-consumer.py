...
tracer_provider = TracerProvider(resource=Resource.create({SERVICE_NAME:'java-app'}))
...
class TreeNode:
    def __init__(self,span_id,parent_id,span_data):
...
def send_traces_to_jaeger(node,traceId):
    ...
    with tracer_provider.get_tracer(__name__).start_as_current_span(node.span_data['name']) as span:
...
def find_parent(root_node,span):
    ...
    if root_node.span_id == span[1]:
        new_node = TreeNode(span[0],span[1],span[2])
...
def create_Tree(dic):
    for trace_id in dic:
        ...
        for span in spans:
            if span[1]=='':
                root = span[0]
                root_node = TreeNode(root,span[1],span[2])
                spans.remove(span)
                break
        curr_node = root_node
        while len(spans)>0:
...
def inject_trace_data_into_tracer(trace_data):
    ...
    for span_data in scopeSpans:
        for span in span_data['spans']:
            ...
            if trace_id in dic:
                dic[trace_id].append([span_id,parent_id,{'name':span['name'],'kind':span['kind'],'start_time':span['startTimeUnixNano'],'end_time':span['endTimeUnixNano']}])
            else:
                dic[trace_id] = [[span_id,parent_id,{'name':span['name'],'kind':span['kind'],'start_time':span['startTimeUnixNano'],'end_time':span['endTimeUnixNano']}]]
